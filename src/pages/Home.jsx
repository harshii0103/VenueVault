...
function Logo({ light = false }) {
  return (
    <a href="#" className="flex items-center gap-2.5" aria-label="VenueVault home">
        ...
        <nav className="mx-auto flex h-20 max-w-[1180px] items-center justify-between px-5 lg:px-8" aria-label="Main navigation">
          ...
          <div className="hidden items-center gap-8 md:flex">
            {["Home", "Venues", "About", "Contact"].map((item, index) => (
              <a key={item} href={`#${item.toLowerCase()}`} className={`text-sm font-medium transition hover:text-white ${index === 0 ? "text-white" : "text-slate-300"}`}>
                {item}
              </a>
            ))}
          </div>
          <div className="flex items-center gap-2.5">
            <a href="#login" className="hidden rounded-lg px-4 py-2.5 text-sm font-semibold text-white transition hover:bg-white/10 sm:block">Login</a>
            <a href="#signup" className="rounded-lg bg-[#a42b43] px-4 py-2.5 text-sm font-semibold text-white shadow-lg shadow-black/15 transition hover:bg-[#b8334d]">Sign Up</a>
          </div>
        </nav>
      </header>
      ...
      <main>
          ...
          <div className="relative mx-auto grid max-w-[1180px] items-center gap-16 px-5 lg:grid-cols-[1.02fr_.98fr] lg:px-8">
            <div className="max-w-2xl">
              ...
              <div className="mt-9 flex flex-wrap gap-3">
                <a href="#venues" className="group inline-flex items-center gap-2 rounded-xl bg-[#a42b43] px-6 py-3.5 text-sm font-bold text-white shadow-xl shadow-black/20 transition hover:-translate-y-0.5 hover:bg-[#b8334d]">
                  Student Portal <Icon name="arrow" className="h-4 w-4 transition group-hover:translate-x-0.5" />
                </a>
                <a href="#admin" className="inline-flex items-center gap-2 rounded-xl border border-white/25 bg-white/[.04] px-6 py-3.5 text-sm font-bold text-white transition hover:bg-white/10">
                  Admin Portal <Icon name="chevron" className="h-4 w-4" />
                </a>
              </div>
            ...
            <div className="relative mx-auto w-full max-w-[550px]">
              <div className="absolute -inset-5 rounded-[2rem] bg-[#9f263d]/15 blur-2xl" />
              <div className="relative overflow-hidden rounded-[1.4rem] border border-white/15 bg-[#f8fafc] shadow-2xl shadow-black/30">
                <div className="flex items-center justify-between border-b border-slate-200 bg-white px-5 py-4">
                  <div>
                    <p className="text-[10px] font-bold tracking-[.15em] text-[#9f263d]">VENUEVAULT</p>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="grid h-8 w-8 place-items-center rounded-full bg-[#13203e] text-[10px] font-bold">AD</span>
                  </div>
                </div>
                  ...
                  <div className="min-w-0 p-4 sm:p-6">
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-[10px] text-slate-400">Campus venue calendar</p>
                      </div>
                      <button className="rounded-lg bg-[#9f263d] px-3 py-2 text-[10px] font-bold text-white">+ New booking</button>
                    </div>
                    <div className="mt-5 grid grid-cols-5 gap-1.5 text-center">
                      {["MON", "TUE", "WED", "THU", "FRI"].map((day) => <span key={day} className="pb-2 text-[8px] font-bold text-slate-400">{day}</span>)}
                      {[12, 13, 14, 15, 16, 19, 20, 21, 22, 23].map((date) => (
                        <div key={date} className={`relative h-12 rounded-md border pt-2 text-[10px] font-semibold ${date === 15 ? "border-[#9f263d] bg-[#9f263d] text-white" : "border-slate-200 bg-white text-slate-500"}`}>
                          {date}
                    ...
                    <div className="mt-4 flex items-center justify-between rounded-lg border border-emerald-100 bg-emerald-50 px-3 py-2.5">
                      <span className="flex items-center gap-2 text-[10px] font-bold text-emerald-800"><span className="grid h-5 w-5 place-items-center rounded-full bg-emerald-500 text-white"><Icon name="check" className="h-3 w-3" /></span>No booking conflicts</span>
              ...
              <div className="absolute -bottom-7 -left-5 hidden items-center gap-3 rounded-xl border border-slate-200 bg-white p-3.5 text-[#13203e] shadow-xl sm:flex">
                ...
                <div><p className="text-[9px] text-slate-400">AI RECOMMENDATION</p><p className="text-xs font-bold">NSB Hall is your best match</p></div>
              </div>
            </div>
          </div>
        </section>
        ...
        <section id="about" className="mx-auto max-w-[1180px] px-5 py-24 lg:px-8 lg:py-32">
          ...
          <div className="mt-12 grid gap-5 md:grid-cols-3">
            {features.map((feature, index) => (
              <article key={feature.title} className="feature-card group relative overflow-hidden rounded-2xl border border-slate-200 bg-white p-7 shadow-sm transition hover:-translate-y-1 hover:shadow-xl hover:shadow-slate-900/[.06]">
                <span className="absolute right-5 top-4 font-display text-5xl font-bold text-slate-100">0{index + 1}</span>
                <span className="relative grid h-12 w-12 place-items-center rounded-xl bg-[#13203e] text-white"><Icon name={feature.icon} /></span>
                <p className="mt-7 text-[10px] font-bold tracking-[.14em] text-[#a42b43]">{feature.label}</p>
                ...
                <a href="#" className="mt-6 inline-flex items-center gap-1.5 text-xs font-bold text-[#13203e]">Learn more <Icon name="arrow" className="h-3.5 w-3.5" /></a>
              </article>
            ))}
          </div>
        </section>
          ...
          <div className="mx-auto max-w-[1180px] px-5 lg:px-8">
            <div className="flex flex-col justify-between gap-5 sm:flex-row sm:items-end">
              <div>
                ...
                <h2 className="section-title mt-3">Popular venues</h2>
                <p className="mt-3 text-sm text-slate-500">Find the right setting for your next campus event.</p>
              </div>
            ...
            <div className="mt-10 grid gap-6 md:grid-cols-3">
              {venues.map((venue) => (
                <article key={venue.name} className="group overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-xl hover:shadow-slate-900/[.07]">
                  <div className="relative h-56 overflow-hidden bg-slate-200">
                    ...
                    <span className="absolute right-4 top-4 flex items-center gap-1.5 rounded-full bg-white/95 px-3 py-1.5 text-[10px] font-bold text-emerald-700 shadow-sm"><span className="h-1.5 w-1.5 rounded-full bg-emerald-500" />AVAILABLE</span>
                  ...
                  <div className="flex items-center justify-between p-5">
                    ...
                    <a href="#" aria-label={`View ${venue.name}`} className="grid h-10 w-10 place-items-center rounded-full border border-slate-200 text-[#13203e] transition group-hover:border-[#9f263d] group-hover:bg-[#9f263d] group-hover:text-white"><Icon name="arrow" className="h-4 w-4" /></a>
                  </div>
                </article>
              ))}
            </div>
            ...
            <div className="relative mt-14 grid gap-8 sm:grid-cols-2 lg:grid-cols-4">
              ...
              {[
                ...
                <div key={title} className="relative text-center">
                  ...
                  <span className="absolute left-[calc(50%+18px)] top-0 grid h-5 w-5 place-items-center rounded-full bg-[#a42b43] text-[9px] font-bold text-white">{index + 1}</span>
        ...
        <section className="px-5 pb-24 lg:px-8 lg:pb-28">
          <div className="relative mx-auto flex max-w-[1120px] flex-col items-center justify-between gap-7 overflow-hidden rounded-3xl bg-[#9f263d] px-7 py-12 text-center text-white sm:px-12 lg:flex-row lg:text-left">
            ...
            <a href="#" className="relative inline-flex shrink-0 items-center gap-2 rounded-xl bg-white px-6 py-3.5 text-sm font-bold text-[#8f2036] shadow-lg transition hover:-translate-y-0.5">Browse venues <Icon name="arrow" className="h-4 w-4" /></a>
          </div>
        </section>
      </main>

      <footer id="contact" className="border-t-4 border-[#9f263d] bg-[#0c1730] text-slate-300">
        <div className="mx-auto grid max-w-[1180px] gap-10 px-5 py-14 sm:grid-cols-2 lg:grid-cols-[1.5fr_.7fr_.7fr_1fr] lg:px-8">
          <div>
            <Logo light />
          ...
          <div><h3 className="text-xs font-bold tracking-[.12em] text-white">PLATFORM</h3><div className="mt-5 flex flex-col gap-3 text-sm text-slate-400"><a href="#">Student portal</a><a href="#">Admin portal</a><a href="#">Venue policies</a><a href="#">Support</a></div></div>
          <div><h3 className="text-xs font-bold tracking-[.12em] text-white">CONTACT</h3><div className="mt-5 flex flex-col gap-4 text-sm text-slate-400"><p className="flex gap-2.5"><Icon name="map" className="mt-0.5 h-4 w-4 shrink-0 text-[#d5576e]" />Campus Administration Block</p><a href="mailto:help@venuevault.edu" className="flex gap-2.5"><Icon name="mail" className="h-4 w-4 shrink-0 text-[#d5576e]" />help@venuevault.edu</a></div></div>
        </div>
        <div className="border-t border-white/[.08]">
          <div className="mx-auto flex max-w-[1180px] flex-col justify-between gap-2 px-5 py-5 text-[11px] text-slate-500 sm:flex-row lg:px-8"><p>© 2025 VenueVault. All rights reserved.</p><p>Built for a smarter campus.</p></div>
        </div>
      </footer>
    </div>
  );
